    ...
    public function store(Request $request)
{
    $request->validate([
        ...
        'penjualan_id' => 'nullable|exists:penjualan,id' // Tambahkan validasi ini
    ]);

    DB::transaction(function () use ($request) {

        // 1. Ambil transaksi berdasarkan ID yang dikirim form.
        // Jika tidak ada ID yang dikirim, cari transaksi milik user yang statusnya OPEN.
        if ($request->filled('penjualan_id')) {
            $sale = Penjualan::where('id', $request->penjualan_id)->firstOrFail();
        } else {
        ...
        if ($product->stok < $request->quantity) {
            return redirect()->back()->with('errors', 'Stok produk tidak mencukupi');
        }
