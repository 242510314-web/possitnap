...
<div class="row">
    ...
    <form method="POST" action="{{ route('itempenjualan.store') }}" class="row mb-2">
        ...
        {{-- PASTIKAN INPUT INI ADA --}}
        <input type="hidden" name="penjualan_id" value="{{ $sale->id }}">
        <input type="hidden" name="product_id" value="{{ $product->id }}">

        <div class="col-7">
            {{-- Ubah button jadi type="button" atau gunakan div agar tidak men-submit form secara tak sengaja --}}
            <div class="btn btn-outline-primary w-100 text-start p-2 {{ $sale->status === 'COMPLETED' ? 'disabled' : '' }}">
    ...
    <div class="card">
        ...
        <div class="card-footer">
            ...
            <form method="POST"
                    ...
                    onsubmit="return confirm('Yakin ingin checkout?')" class="mt-2" id="payment-form">
                @csrf
                @method('PUT')
                <select name="payment_method" class="form-select mb-2" id="payment-method" required>
                ...
                <div id="cash-payment-fields">
                    <label for="uang-dibayar" class="form-label">Uang Dibayar</label>
                    <input type="number" name="uang_dibayar" id="uang-dibayar"
                           class="form-control mb-2" min="0" step="1"
...
<script>
    ...
    const paidAmount = document.getElementById('uang-dibayar');
    ...
    function updatePaymentFields() {
        ...
        const change = isCash ? Math.max((Number(paidAmount.value) || 0) - totalAmount, 0) : 0;
        changeAmount.textContent = 'Rp ' + change.toLocaleString('id-ID');
    }

    paymentMethod.addEventListener('change', updatePaymentFields);
    paidAmount.addEventListener('input', updatePaymentFields);
    updatePaymentFields();
</script>
@endsection